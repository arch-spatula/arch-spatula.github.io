...
async function reqJsonPlaceholder() {
  const promises = Array.from({ length: 100 }, (_, idx) =>
    fetch(`https://jsonplaceholder.typicode.com/posts/${idx + 1}`).then((response) => response.json()),
  );
...
![100개-후-페이지-전환](100개-후-페이지-전환.png)

![실제-100개-요청](실제-100개-요청.gif)
...
![6개-후-페이지-전환](6개-후-페이지-전환.png)

![6개-단위-요청](6개-단위-요청.gif)
