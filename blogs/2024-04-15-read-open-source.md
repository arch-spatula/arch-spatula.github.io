...
읽고자 하는 소스가 라이브러인지 프레임워크인지 구분하고 엔트리포인트를 찾는 것이 먼저입니다.

기술의 성격을 이해하는 것이 중요합니다. 일반적으로 패키지 매니저가 없는 언어라면 어떻게 읽을 것인가? 또 패키지를 전달하는 과정을 어떻게 되는가?

일반적으로 라이브러리 코드를 읽는다고 하면 그 라이브러리의 여러 기능들을 호출해서 사용합니다. 이 기능을 호출하는 import가 어떻게 묶여있는지 파악해야 합니다. 보통은 각각의 함수 단위로 export가 되어 있으면 각각의 함수별로 구현을 읽을 수 있을 것입니다.

<!--
여기에는 예시를 추가하는 것이 적절해보입니다. toss의 es-toolkit을 다루는 것이 좋을 것 같습니다.
 -->
