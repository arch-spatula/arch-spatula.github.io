...
<!--
@todo: mermaid 다이어그램 빌드 스탭에 처리하도록 만들기
@todo: 다이어그램을 2025-11-23-new-blog.md에 정리하기
@todo: 이미지를 마크다운 폴더에 같이 넣어둔 로직 작성하기
-->
...
flowchart TB
    ...
    subgraph Prepare["준비 단계"]
        C[마크다운 파일 목록화] --> D[dist 폴더 초기화]
        D --> E[asset 폴더 복사]
        E --> F[이미지 파일 복사]
        F --> G[client TS → JS 빌드]
    end

    subgraph Meta["메타데이터 처리"]
        H[마크다운 파일 읽기] --> I[메타데이터 추출]
        I --> J{draft 여부}
        J -->|draft: true| K[스킵]
        J -->|draft: false| L[metaJson에 추가]
        L --> M[태그 정보 수집]
    end

    subgraph Render["렌더링 단계"]
        N[search 템플릿 렌더링] --> O[각 포스트 처리]
        O --> P[main 페이지 생성]
        P --> Q[404 페이지 생성]
    end
...
flowchart LR
    ...
    subgraph Process["render 함수 처리 순서"]
        A["1️⃣ renderConditional<br/>조건문 처리"] --> B["2️⃣ renderEach<br/>반복문 처리"]
        B --> C["3️⃣ renderTemplate<br/>플레이스홀더 치환"]
    end

    subgraph Syntax["지원 문법"]
        S1["{{#if KEY}}...{{/if}}"]
        S2["{{#each ARRAY}}...{{/each}}"]
        S3["{{PLACEHOLDER}}"]
    end
...
```mermaid
flowchart TB
    subgraph Templates["템플릿 파일들"]
        APP["app.html<br/>(기본 레이아웃)"]
        POST["post.html<br/>(포스트 내용)"]
        MAIN["main.html<br/>(블로그 목록)"]
        SEARCH["search.html<br/>(검색 팝업)"]
    end

    subgraph PostPage["포스트 페이지 생성"]
        MD[마크다운 파일] --> HTML[HTML 변환]
        HTML --> POST_RENDER["post 템플릿 렌더링<br/>{{content}}, {{tags}}, {{toc}}"]
        POST_RENDER --> APP_RENDER["app 템플릿 렌더링<br/>{{body}}, {{search}}"]
    end

    subgraph MainPage["메인 페이지 생성"]
        META[metaJson] --> MAIN_RENDER["main 템플릿 렌더링<br/>{{#each posts}}"]
        MAIN_RENDER --> APP_MAIN["app 템플릿 렌더링<br/>{{body}}, {{search}}"]
    end
...
### 4. 마크다운 → HTML 변환 파이프라인

```mermaid
flowchart LR
    subgraph Remark["Remark 파싱"]
        A[마크다운 원본] --> B[remark-parse]
        B --> C[remark-gfm]
        C --> D[remark-directive]
        D --> E[remarkCallout]
    end

    subgraph Rehype["Rehype 변환"]
        E --> F[remark2rehype]
        F --> G[rehype-slug]
...
| 템플릿        | 플레이스홀더                   | 설명               |
| ------------- | ------------------------------ | ------------------ |
| `app.html`    | `{{body}}`                     | 메인 콘텐츠 영역   |
|               | `{{search}}`                   | 검색 팝업 HTML     |
|               | `{{title}}`, `{{description}}` | SEO 메타 정보      |
| `post.html`   | `{{content}}`                  | 마크다운 변환 HTML |
|               | `{{#each tags}}`               | 태그 목록 반복     |
|               | `{{#each toc}}`                | 목차 반복          |
|               | `{{#if previousPost}}`         | 이전 글 조건부     |
| `main.html`   | `{{#each posts}}`              | 블로그 목록 반복   |
| `search.html` | `{{#each tags}}`               | 태그 필터 목록     |
|               | `{{#each posts}}`              | 검색 결과 목록     |

이 다이어그램들이 프로젝트의 템플릿 처리 방식을 이해하는 데 도움이 되셨으면 좋겠습니다! 🎨
