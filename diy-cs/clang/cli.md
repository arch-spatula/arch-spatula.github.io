...
`fgets`는 오직 개행문자(`\n`)를 기준으로 입력의 종료로 간주합니다. 이렇게 되면 띄어쓰기(` `)와 탭(`\t`)을 표준입력에서 표현할 수 있게 됩니다.[^6]
...
함수 시그니쳐를 자세히 볼 필요가 있습니다.

현재 스트림부터 첫번째 개행문자(`\n`)라고 IBM 스펙에 작성되었습니다.[^7] 이것은 위에 제가 찾은 자료의 설명과 같습니다. 스트림의 끝 혹은 `n - 1`까지라고 합니다. 버퍼사이즈를 고정시키고 있습니다. 함수는 결과를 첫번째 인자인 `string` 주소에 쓰기를 하고 뒤에 개행문자(`\n`)를 추가한다고 합니다.

예외도 설명한 것 같습니다.

- `string` 읽기에서 줄바꿈을 포함합니다.
- `n == 1`인 경우 `string`은 비어있습니다.

이런 스팩들로 보면 `fgets`는 보완된 함수입니다. 기존 `gets`가 보안 취약점이 많아서 `gcc` 컴파일러는 `gets` 대신에 `fgets`를 사용할 것을 권장합니다. 기존 `gets`는 버퍼 오버플로우 공격이 쉬운 편입니다. 하지만 `fgets`의 경우 함수를 호출할 때 최대 버퍼 크기를 정하기 때문에 상대적으로 안전합니다. 또 직접 제어할 수 있다는 혜택은 유지됩니다.

## 스트림이란 무엇인가?

스트림은 물줄기 혹은 개울이라고 변역합니다. 흐르는 물인데 컴퓨터 과학의 경우 어떤 데이터의 흐름에 해당합니다.

버퍼랑 대비되는 개념처럼 생각하는데 별개의 개념이 더 올바릅니다.

하드웨어상 버퍼가 있어야 할 모든 위치에는 스트림이 있습니다. 파일 읽기, 네트워크 소켓 흐르고 있는 데이터를 지칭하는 추상적인 개념입니다.

[스트림 (컴퓨팅)](<https://ko.wikipedia.org/wiki/%EC%8A%A4%ED%8A%B8%EB%A6%BC_(%EC%BB%B4%ED%93%A8%ED%8C%85)>)
...
[^6]: [fgets - 모두의 코드](https://modoocode.com/38)
[^7]: [fgets() — 스트링 읽기 - IBM](https://www.ibm.com/docs/ko/i/7.5?topic=functions-fgets-read-string)
