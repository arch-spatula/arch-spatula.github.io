...
[C를 배우기 전에 반드시 알아야 할 것들](https://www.yes24.com/Product/Goods/17967251)을 먼저 보면 도움될 것입니다.

<!-- TODO: 메모리는 종이

메모리를 종이처럼 생각하기 바랍니다.

CPU가 계산기라면 메모리는 계산 작업을 수행할 일렬의 명령들입니다.

컴퓨터가 컴퓨터일 수 있는 것은 이런 프로그래밍이 가능해야 컴퓨터입니다.

프로그래밍은 순서에 따라 작업을 수행할 명령을 작성하는 행위이고 메모리에 명령을 올리고 CPU가 순서대로 처리합니다.

이것은 폰노이만 아키텍쳐의 본질입니다.

이런 말들은 구체적인 그림을 봐야 이해가 됩니다.

-->

## 하드웨어 지식을 알아야 하는 이유

- 모든 소프트웨어는 하드웨어에 의존합니다. 본인이 의존하고 있는 환경을 이해해야 환경을 개선하거나 활용할 수 있습니다.
- 하드웨어 성능이 나쁘면 소프트웨어 성능도 필연적으로 나빠질수밖에 없습니다. 반대로 하드웨어 성능은 평범한데 소프트웨어 성능이 나쁜 경우가 현대 소프트웨어 개발의 일상입니다.
- 성능을 이해하려면 하드웨어를 반드시 이해해야 합니다.
- 웹분야는 성능이 나빠지는 경우가 많지 않습니다. 하지만 보안규정 때문에 나빠지거나 특정 비즈니스 도메인 혹은 비즈니스 로직을 구현하면서 나빠지는 경우도 많습니다.
- 게임분야는 소프트웨어 분야에서 기술적 난이도가 높은편에 속합니다. 성능문제 일찍 도달합니다.
...
기호를 표시하는 무한한 테이프와 테이프를 보고 기호를 해석하고 작업을 수행하는 기계를 보고 튜링머신이라고 합니다.

## 폰노이만 아키텍쳐 요약

컴퓨터란 무엇인가? 본질은 계산하는 기계입니다. 주판은 그냥 계산기에 불과합니다. 컴퓨터는 이 계산이라는 작업을 기계가 수행할 수 있는 설계입니다.

컴퓨터는 CPU가 실제 계산작업을 처리합니다. 하지만 이 계산을 수행하기 위해서 메모리에서 옵트코드라는 계산 명령과 오퍼랜드라는 계산 대상을 갖고 있어야 합니다.

컴퓨터를 컴퓨터로 만들어주는 것은 이런 작업의 순서를 제어할 수 있게 프로그래밍이 가능해야 합니다.

이런 이유로 프로그램의 라인 넘버를 표현할 프로그램 카운터가 필요합니다.
