...
## 좁은 화면 대응 — 반영한 요구와 구현 기준

- 기기 종류나 마우스 유무가 아니라 팝업을 사용하기 어려운 좁은 화면을 기준으로 전체 화면 검색을 제공한다.
- 제공된 X SVG를 기존 아이콘처럼 `app/asset/x.svg` 파일로 추가하고, 검색 화면 왼쪽 상단에 닫기 버튼을 둔다. 접근성 이름은 “검색 닫기”다.
- 구현 기준은 너비 **600 CSS px 이하**다. 기존 `50vw` 설정에서 내용 너비가 300px 이하가 되는 구간을 기준으로 선택했다. 사용자와 합의된 기기 분류 기준이 아니라 이번 레이아웃의 전환값이다.
- 전체 화면에서는 바깥 여백과 모서리 둥글기를 제거한다. 닫기 버튼의 클릭 영역은 44 × 44px이며 내부 여백은 12px이다.
- 검색 입력과 닫기 버튼을 유지하고, 태그 영역은 화면 높이의 최대 25%로 제한한다. 글 목록은 남은 공간에서 스크롤한다. 이 비율은 구현 과정의 선택이다.
- X 버튼은 기존 해시 닫기 방식으로 검색을 닫고 검색 버튼으로 포커스를 돌린다. 브라우저 뒤로가기 이력 정책은 이번 변경에 포함하지 않는다.

수용 기준: 360px, 390px, 600px에서는 팝업 외곽이 뷰포트를 채우고 왼쪽 상단 X 버튼을 클릭하거나 Enter로 활성화하면 닫힌다. 601px에서는 기존 팝업을 유지하고 X 버튼을 숨긴다. 태그와 글 목록에 가로 스크롤이 생기지 않아야 한다.

검증: [Playwright 테스트](../../tests/mobile-search-popup.spec.ts) 4개 및 기존 검색 Vitest 16개 통과. 실제 모바일 가상 키보드는 미검증이다. 전체 빌드는 Mermaid 브라우저 컨텍스트 종료 오류로 실패했으며, 생성된 로컬 검색 페이지에서 UI를 검증했다.

![360px 전체 화면 검색](../screenshots/mobile-popup/after-360x800.png)
